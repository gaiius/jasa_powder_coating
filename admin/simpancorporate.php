	...
	if (!empty($_FILES["nama_file"]["tmp_name"]))
{
	...
	{			
		...
		if (move_uploaded_file($_FILES['nama_file']['tmp_name'], $foto))
			 {
			$sql="insert into corporate(judul,keterangan,nama_file,tanggal) values 
	...
	else 
	{
		...
		$keterangan=$_POST['keterangan'];
		$tgl=date('Y-m-d');
			$sql="insert into corporate(judul,keterangan,tanggal) values 
...
if ($sql){
	...
	?>
    <script>document.location='admin.php?page=corporate';</script>
<?php 
	}
	else{
		echo"gagal";
